...
g2=conv2(f,edgey,'same'); % Devuelve una secci贸n de la convoluci贸n, en este caso el centro. (Borde Y)
imshow(g2,[-10,10]) % Muestra una imagen en escala de grises con rango de visualizaci贸n de -10 a 10 de la convoluci贸n central.
figure(2) % Inicializa otra figura para que no se borre la anterior
subplot(1,2,1) % Divide la figure(2) en cuadr铆culas (subgr谩ficas)
imshow(g1,[-10,10]) % Muestra una imagen en escala de grises con rango de visualizaci贸n de -10 a 10 de la convoluci贸n central.
subplot(1,2,2) % Divide la figure(2) en cuadr铆culas (subgr谩ficas)
imshow(g2,[-10,10]) % Muestra una imagen en escala de grises con rango de visualizaci贸n de -10 a 10 de la convoluci贸n central.
...
gx=conv2(f,edgex,'same'); %Devuelve una secci贸n de la convoluci贸n, en este caso del centro (Borde x)
gy=conv2(f,edgey,'same');%Devuelve una secci贸n de la convoluci贸n, en este caso del centro (Borde y)
...
noiseimage = conv2(f,noisemask,'same'); % Devuelve la parte central de la convoluci贸n, la misma medida que f.
noisevariance = mean2(noiseimage.^2); % Calcula la varianza mediante la media de la convoluci贸n.
noisestd = sqrt(noisevariance/2); % Calcula la derivaci贸n estandar sacando la raiz cuadrada
